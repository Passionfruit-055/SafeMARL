...
class GetDataSet(object):
    ...
    def load_data(self, isIID):
        ...
        train_images, test_images = train_data.astype(np.float32).reshape(self.train_data_size, 3, 32,
                                                                          32), test_data.astype(np.float32).reshape(
            self.test_data_size, 3, 32, 32)
        ...
        # # 数组对应元素位置相乘
        # train_images = np.multiply(train_images, 1.0 / 255.0).reshape(self.train_data_size, 3, 32, 32)
        # # train_images = torch.from_numpy(train_images)
        # # test_images = test_images.astype(np.float32)
        # test_images = test_data.astype(np.float32)
        # test_images = np.multiply(test_images, 1.0 / 255.0).reshape(self.test_data_size, 3, 32, 32)
        ...
        #     self.train_label = train_labels[order]
        #
        # test_indices = np.random.choice(len(test_images),1000, replace=False)
        # self.test_data = test_images[test_indices]
        # self.test_label = test_labels[test_indices]
    def load_mnist(self, isIID):
        transform_mnist = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.1307,), (0.3081,))
        ])

        train_set = torchvision.datasets.MNIST(root='./env/FederatedUpload/data/MNIST_cp', train=True, download=False, transform=transform_mnist)
        test_set = torchvision.datasets.MNIST(root= './env/FederatedUpload/data/MNIST_cp', train=False, download=False, transform=transform_mnist)
